...
## Sample Project: TalentLens

A personal **Skill Tracker / Learning Dashboard** — Go (Gin) backend + MySQL + Nginx + vanilla HTML/CSS/JS frontend, all wired up with Docker Compose. The app is already in `TalentLens/` — the course doesn't teach the app, it teaches how to ship it.
...
- **Hands-on**: Push TalentLens to a GitHub repo. Write a one-step `hello.yml` that prints `echo "Hello Actions"` on every push. See it run in the Actions tab.
    ...
    steps:
      ...
      - uses: docker/build-push-action@v7
        with:
          ...
          push: true
          tags: |
            ${{ secrets.DOCKERHUB_USERNAME }}/TalentLens-backend:${{ github.sha }}
            ${{ secrets.DOCKERHUB_USERNAME }}/TalentLens-backend:latest
```
...
- Docker Hub image name: `<dockerhub-username>/TalentLens-backend`
...
- user_data installs Docker + Compose plugin, clones the public repo into `/home/ubuntu/TalentLens`, writes `.env` with `DOCKERHUB_USERNAME` injected by Terraform
- Compose change: backend service uses `image: ${DOCKERHUB_USERNAME}/TalentLens-backend:latest` (not `build:`)
- The CD workflow assumes `~/TalentLens` exists with `docker-compose.yml` and `.env` already in place. Each deploy: `docker compose pull && docker compose up -d`.
